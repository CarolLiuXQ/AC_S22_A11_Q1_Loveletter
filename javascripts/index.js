...
searchButton.addEventListener('click' || 'keypress', function clickedSearchButton(event) {
  //防止searchButton重整頁面
  event.preventDefault()
  const searchInputCleansed = searchInput.value.toLowerCase().split(' ').join('')
  filterFriend = peopleData.filter(item => item.name.toLowerCase().includes(searchInputCleansed) || item.surname.toLowerCase().includes(searchInputCleansed))
  searchRender()
})
